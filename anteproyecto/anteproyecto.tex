...
con reducciones más claras en la velocidad de transmisión. En este documento,
el término \textit{medidas} se usa en un sentido amplio: incluye
intervenciones sanitarias, regulatorias y de comportamiento, tales como
aislamiento, uso de mascarillas, restricciones de aforo, campañas de
vacunación, pruebas diagnósticas o combinaciones de estas. Esta aclaración
evita restringir el análisis a medidas no farmacológicas y mantiene la
coherencia entre la pregunta de investigación, los objetivos y la estrategia
empírica.

Este enfoque es pertinente porque permite vincular la simulación con evidencia
empírica. Para ello, el estudio puede apoyarse en fuentes comparables de series
epidemiológicas y de registro de políticas públicas, como \textit{Our World in
Data} \cite{owid-coronavirus} y el \textit{Oxford COVID-19 Government Response
Tracker} \cite{Hale2021}. Al poner en relación las curvas generadas por el
modelo con la evolución real de los casos y con la cronología de las medidas
aplicadas, se podrá analizar qué intervenciones o combinaciones de
intervenciones se asocian con cambios relevantes en la transmisión. De este
modo, el proyecto combina modelado computacional, análisis de datos y
evaluación comparativa de medidas sanitarias y de política pública.
...
Evaluar la capacidad explicativa de un modelo de simulación Monte Carlo, basado
...
incertidumbre del proceso de contagio. En los modelos deterministas, la
evolución del sistema queda fijada por ecuaciones diferenciales y parámetros
medios; en cambio, los modelos estocásticos consideran que la transmisión
...
El trabajo de Xie \cite{Xie2020} se ubica dentro de esa tradición estocástica.
Su propuesta modela la propagación de COVID-19 como un proceso de puntos en el
...
distribución de Poisson con media dependiente de una tasa temporal \(R_t\),
...
epidemiológicos comparables, y el \textit{Oxford COVID-19 Government Response
Tracker} \cite{Hale2021}, que codifica la evolución de distintas políticas
públicas.

El proyecto también se apoya en la idea de validación por contraste entre
simulación y observación. Un modelo Monte Carlo no debe entenderse solo como
una herramienta para producir curvas hipotéticas, sino como un mecanismo para
evaluar hasta qué punto una estructura probabilística logra reproducir patrones
reales. Si el modelo captura adecuadamente cambios en la tendencia de contagios
...
\begin{itemize}
  ...
  \item El proceso se repite durante toda la ventana temporal de estudio para
        ...
        acumulados.
  \item La simulación se ejecuta múltiples veces para obtener bandas de
        variación y no una única curva rígida.
\end{itemize}
